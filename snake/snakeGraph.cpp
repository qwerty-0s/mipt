...
extern std::string game_result;
extern bool game_started;
extern bool game_over;

Game_Window::Game_Window(int w, int h, snake *p1, snake *p2, int cell_size_)
    : Fl_Window(w, h, "Snake Game"), player1(p1), player2(p2),
      cell_size(cell_size_), meal(new food(w / cell_size_, h / cell_size_)) {
  end();
  take_focus();
}

Game_Window::~Game_Window() { delete meal; }

int Game_Window::handle(int event) {
  if (event == FL_KEYDOWN || event == FL_SHORTCUT) {
    int key = Fl::event_key();
    const char *txt = Fl::event_text();
    char ch = (txt && txt[0]) ? static_cast<char>(std::tolower(
    ...
    case FL_Down:
      if (player1)
        player1->set_direction(snake::DOWN);
      return 1;
    case FL_Left:
      if (player1)
        player1->set_direction(snake::LEFT);
      return 1;
    case FL_Right:
      if (player1)
        player1->set_direction(snake::RIGHT);
      return 1;
    case FL_Escape:
      hide();
      return 1;
    ...
    if (ch) {
      ...
      case 'w':
        if (player2)
          player2->set_direction(snake::UP);
        return 1;
      case 's':
        if (player2)
          player2->set_direction(snake::DOWN);
        return 1;
      case 'a':
        if (player2)
          player2->set_direction(snake::LEFT);
        return 1;
      case 'd':
        if (player2)
          player2->set_direction(snake::RIGHT);
        return 1;
      default:
        break;
      }
    ...
    printf("Key pressed: key=%d orig=%d text='%c' (code %d)\n", key,
           Fl::event_original_key(), ch ? ch : '?',
           ch ? static_cast<int>(ch) : -1);
    return 0;
  }
...
void Game_Window::draw() {
  ...
  if (!game_started && !game_result.empty()) {
    fl_font(FL_HELVETICA_BOLD, 72);
    fl_color(FL_YELLOW);

    int text_width, text_height;
    fl_measure(game_result.c_str(), text_width, text_height, 0);
    int x = (w() - text_width) / 2;
    int y = h() / 2;

    fl_draw(game_result.c_str(), x, y);
  }

  if (game_over && !game_result.empty()) {
...
int Game_Window::get_cell_size() { return cell_size; }

snake *Game_Window::get_player1() { return player1; }

snake *Game_Window::get_player2() { return player2; }

food *Game_Window::get_meal() { return meal; }
